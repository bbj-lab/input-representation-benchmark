...
#
# =============================================================================
# Runner (Stage 0E; tier2q; CPU-only): executes the Nth line of a Stage0E jobfile
#
# Rationale:
...
if [[ -z "$JOBFILE" ]]; then
  ...
  echo "Usage: sbatch --array=0-N slurm/02_run_stage0e_tier2q_tokenize.sh <jobfile>" >&2
  exit 1
fi
if [[ ! -f "$JOBFILE" ]]; then

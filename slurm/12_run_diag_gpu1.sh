...
# =============================================================================
# Runner (Diagnostics; 1 GPU): executes the Nth line of a jobfile
#
# Lighter than the 2-GPU training runner: single GPU, less memory, shorter
# wall time. Designed for inference-only diagnostic analyses.
# =============================================================================
...
if [[ -z "$JOBFILE" ]]; then
  ...
  echo "Usage: sbatch --array=0-N slurm/12_run_diag_gpu1.sh <jobfile>" >&2
  exit 1
fi
if [[ ! -f "$JOBFILE" ]]; then

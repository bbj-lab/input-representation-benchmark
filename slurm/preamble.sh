...
# Project paths
IRB_HOME="${hm}/input-representation-benchmark"
FMS_EHRS_HOME="${hm}/fms-ehrs"
DATA_DIR="${IRB_HOME}/benchmarks/mimic-meds-extraction/data/meds"
MODEL_DIR="${IRB_HOME}/models"
...
name=$(scontrol show job "$SLURM_JOBID" \
    | grep -m 1 "Command=" \
    | cut -d "=" -f2 \
    | xargs -I {} basename {} .sh 2>/dev/null || echo "unknown")
jname=$(scontrol show job "$SLURM_JOBID" \
    | grep -oP 'JobName=\K\S+' 2>/dev/null || echo "unknown")
parent_dir="$(dirname "$(dirname "$(realpath "${BASH_SOURCE[0]}")")")"
export name jname parent_dir

# Activate Python environment
source ~/.bashrc 2>/dev/null
source "${FMS_EHRS_HOME}/venv/bin/activate" 2>/dev/null
...
PYTHONPATH="${IRB_HOME}/src:${FMS_EHRS_HOME}:${PYTHONPATH:-}"

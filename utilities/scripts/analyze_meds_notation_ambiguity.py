...
- This script measures the *pervasiveness* of text-valued events in MEDS and summarizes
  common surface-form variants for a small, explicitly defined lexicon of normalized labels
...
def _label_expr(lex: Lexicon) -> pl.Expr:
    t = pl.col("text_norm")
...
def main(
    *,
    ...
    # (2) Surface-form ambiguity: counts of normalized strings and label groups.
    lex = Lexicon()
    lf_text = (
        lf.filter(has_text)
        .with_columns(_text_norm_expr())
        .with_columns(_label_expr(lex))
    )

    label_counts = (
        lf_text.group_by("label_group")
        .agg(n=pl.len(), n_unique_norm=pl.col("text_norm").n_unique())
        .sort("n", descending=True)
        .collect(streaming=True)
    )

    # Top surface forms per normalized label group (rank within group).
    forms = (
        lf_text.group_by(["label_group", "text_norm"])
        .agg(n=pl.len())
        .with_columns(rank=pl.col("n").rank("dense", descending=True).over("label_group"))
        .filter(pl.col("rank") <= top_k_forms)
        .sort(["label_group", "n"], descending=[False, True])
        .collect(streaming=True)
    )
    ...
    summary = {
        ...
        "label_counts": label_counts.to_dicts(),
    ...
    print(by_domain)
    print("")
    print("== normalized label summary (within text-valued rows) ==")
    print(label_counts)
    print("")
    print(f"== top {top_k_forms} normalized surface forms per label group ==")
    print(forms)
    print("")

...
Why this exists
---------------
An older token-based outcome path inferred several labels from token presence in
full-stay and 24h-truncated timelines. That approach depends on event timing
being preserved in the token sequence.
...
Benchmark counting note
-----------------------
At the benchmark level, `icu_admission` and `prolonged_icu_stay` are counted as
separate binary outcomes. A given experiment uses only one of them as its ICU
endpoint: Experiments 1--2 use `icu_admission`, while Experiment 3 uses
`prolonged_icu_stay`.
...
def _compute_outcomes_from_meds(
    ...
    *,
    ...
    # ICU activity can span multiple segments per admission. Sum those segments
    # instead of taking the envelope from first ICU admission to last discharge.
    icu_events = (
        meds.filter(pl.col("hadm_id").is_not_null())
        .filter(
        ...
        .with_columns(
            is_icu_admission=pl.col("code").str.starts_with("ICU_ADMISSION"),
            is_icu_discharge=pl.col("code").str.starts_with("ICU_DISCHARGE"),
        )
        .sort(["hadm_id", "time"])
        .with_columns(
            icu_segment_id=pl.col("is_icu_admission")
            .cast(pl.Int64)
            .cum_sum()
            .over("hadm_id")
        )
        .filter(pl.col("icu_segment_id") > 0)
    )
    icu_segments = (
        icu_events.group_by(["hadm_id", "icu_segment_id"])
        .agg(
            icu_segment_admission=pl.col("time")
            .filter(pl.col("is_icu_admission"))
            .min(),
            icu_segment_discharge=pl.col("time")
            .filter(pl.col("is_icu_discharge"))
            .min(),
        )
        .filter(pl.col("icu_segment_admission").is_not_null())
        .join(
            discharges.select("hadm_id", "discharge_time"),
            on="hadm_id",
            how="left",
            validate="m:1",
            maintain_order="left",
        ...
        .with_columns(
            icu_segment_end=pl.when(pl.col("icu_segment_discharge").is_not_null())
            .then(pl.col("icu_segment_discharge"))
            .otherwise(pl.col("discharge_time"))
        )
        .with_columns(
            icu_segment_hours=pl.when(pl.col("icu_segment_end").is_not_null())
            .then((pl.col("icu_segment_end") - pl.col("icu_segment_admission")).dt.total_hours())
            .otherwise(0.0)
            .clip(lower_bound=0.0)
        )
    )
    icu = (
        icu_segments.group_by("hadm_id")
        .agg(
            icu_admission_time=pl.col("icu_segment_admission").min(),
            icu_length_of_stay=pl.col("icu_segment_hours").sum(),
        )
    )
    ...
    base = (
        ...
        .join(icu, on="hadm_id", how="left", validate="1:1", maintain_order="left")
    ...
    outcomes = (
        base.with_columns(
            ...
            icu_length_of_stay=pl.col("icu_length_of_stay").fill_null(0.0),

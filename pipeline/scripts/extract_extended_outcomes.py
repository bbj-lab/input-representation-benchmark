#!/usr/bin/env python3
"""
Extended outcome extraction for regression and post-24h binary evaluation.

Companion to `extract_outcomes_meds.py`.  That script produces the core
binary outcomes (mortality, LOS, IMV, and the experiment-specific ICU endpoint:
ICU admission in Experiments 1--2 or prolonged ICU stay in Experiment 3). This script adds
continuous/ordinal outcomes suitable for regression-based probing and
threshold-based post-24h binary outcomes.
...
Usage
-----
python pipeline/scripts/extract_extended_outcomes.py \\
...
# Guardrails for malformed raw vital values. We observed rare MEDS rows such as
# 98109 mmHg DBP and 102115 bpm heart rate that behave like concatenated chart
# values rather than real measurements. Filtering these implausible values fixes
# the downstream regression targets without changing the underlying FM training.
MAP_VALID_RANGE = (10.0, 250.0)
SBP_VALID_RANGE = (20.0, 300.0)
DBP_VALID_RANGE = (20.0, 300.0)
HEART_RATE_VALID_RANGE = (20.0, 300.0)
...
def _vital_extremes(
    ...
    *,
    ...
    if min_valid is not None:
        filtered = filtered.filter(pl.col("numeric_value") >= float(min_valid))
    if max_valid is not None:
        filtered = filtered.filter(pl.col("numeric_value") <= float(max_valid))
    return filtered.group_by("hadm_id").agg(agg_expr.alias(col_name))
...
def _compute_extended_outcomes(
    ...
    *,
    ...
    max_heart_rate = _vital_extremes(
        ...
        "max",
        "max_heart_rate",
        min_valid=HEART_RATE_VALID_RANGE[0],
        max_valid=HEART_RATE_VALID_RANGE[1],
    )
    max_sbp = _vital_extremes(
        meds_post24h,
        SBP_ITEMIDS,
        "max",
        "max_sbp",
        min_valid=SBP_VALID_RANGE[0],
        max_valid=SBP_VALID_RANGE[1],
    )
    max_dbp = _vital_extremes(
        meds_post24h,
        DBP_ITEMIDS,
        "max",
        "max_dbp",
        min_valid=DBP_VALID_RANGE[0],
        max_valid=DBP_VALID_RANGE[1],
    )
    ...
    min_map = _vital_extremes(
        meds_post24h,
        MAP_ITEMIDS,
        "min",
        "min_map",
        min_valid=MAP_VALID_RANGE[0],
        max_valid=MAP_VALID_RANGE[1],
    )
    min_sbp = _vital_extremes(
        meds_post24h,
        SBP_ITEMIDS,
        "min",
        "min_sbp",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Extract extended outcomes (regression + post-24h binary) from MEDS events."
    )
    parser.add_argument(
        "--meds_events_dir",
        type=Path,

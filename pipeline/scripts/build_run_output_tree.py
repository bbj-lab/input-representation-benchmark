#!/usr/bin/env python3
"""
Build a primary run-output tree under ``outputs/runs``.

The goal is to consolidate the real non-deprecated run outputs while preserving
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the primary run output tree with compatibility symlinks."
    )
    parser.add_argument(
        "--run-root",
        type=Path,
        default=repo_root() / "outputs" / "runs",
        help="Primary run output root (default: outputs/runs).",
    )
    parser.add_argument(
        "--dry-run",

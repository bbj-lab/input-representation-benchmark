...
The `*_hadm_ids.csv` inputs are produced by `pipeline/scripts/align_cohorts.py`, which performs a
...
- --splits_dir: directory containing train/val/test hadm lists (from pipeline/scripts/align_cohorts.py):
    - train_hadm_ids.csv
    - val_hadm_ids.csv
    - test_hadm_ids.csv

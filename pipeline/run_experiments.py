...
Usage:
    ...
    python pipeline/run_experiments.py --mode demo

    # Generate for specific experiment
    python pipeline/run_experiments.py --mode demo --exp 1
    python pipeline/run_experiments.py --mode demo --exp 2
    python pipeline/run_experiments.py --mode demo --exp 3

Compatibility:
    python run_experiments.py ... still works through the root wrapper.
...
def make_exp2_configs(
    *,
    ...
    include_affine_continuous: bool = False,
    quantizer: str,
    clinical_anchoring: str,
    fused_discrete: bool,
    ...
    if not discrete_only:
        ...
        if include_affine_continuous:
            configs.extend(
                [
                    ExperimentConfig(
                        quantizer, clinical_anchoring, False, "xval_affine", "time_tokens", "meds"
                    ),
                    ExperimentConfig(
                        quantizer, clinical_anchoring, False, "xval_affine", "time_rope", "meds"
                    ),
                ]
            )
    return configs
...
#
# NOTE:
# The primary Exp3 arms are MEDS-only (native + mapped + null controls) and do
# not depend on any CLIF-side validation workflow.
#
# IMPORTANT:
...
def get_tokenized_data_dir(config: ExperimentConfig) -> str:
    """Get the main tokenized-data root for Stage1/2/3 consumption."""
    if config.data_format in ("meds_icu", "meds_mapped", "meds_randomized", "meds_freqmatched"):
...
def main():
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Include the 2 auxiliary Exp2 xVal-affine reruns used in the paper appendix (default: false).",
    )
    parser.add_argument(
            ...
            if args.exp2_include_discrete:
                ...
                configs.extend(
                    make_exp2_configs(
                        ...
                        include_affine_continuous=False,
            ...
            if not args.exp2_discrete_only:
                ...
                configs.extend(
                    make_exp2_configs(
                        ...
                        include_affine_continuous=bool(args.exp2_include_xval_affine),

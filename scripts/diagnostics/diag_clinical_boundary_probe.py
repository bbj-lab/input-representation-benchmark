...
Usage:
    python scripts/diagnostics/diag_clinical_boundary_probe.py \
        [--models_dir models] \
...
# Source: scripts/augment_clif_labs_with_ref_ranges.py
# ---------------------------------------------------------------------------
...
def extract_embeddings_for_analyte(
    ...
    if analyte_code not in aux:
        return None

    boundaries = aux[analyte_code]  # list of (num_bins - 1) boundary values
...
def plot_probe_results(
    ...
    analyte_names = []
    for a in analytes:
        info = ANALYTE_REFERENCE_RANGES.get(a, {})
        analyte_names.append(info.get("name", a.split("//")[1] if "//" in a else a))
    ax.set_xticklabels(analyte_names, rotation=30, ha="right")
...
def main():
    ...
    for model_label, model_name, quantizer, n_bins in EXP1_MODELS:
        ...
        for analyte_code, ref_info in ANALYTE_REFERENCE_RANGES.items():
            analyte_name = ref_info["name"]

            result = extract_embeddings_for_analyte(
                ckpt_dir, vocab_path, analyte_code, n_bins,
            )
            if result is None:
                print(f"  [{analyte_name}] Not in vocabulary aux, skipping")
                continue
            ...
            n_normal = int((labels == 0).sum())
            n_abnormal = int((labels == 1).sum())
            print(f"  [{analyte_name}] {actual_bins} bins: "
                  f"{n_normal} normal, {n_abnormal} abnormal")

            if args.dry_run:
                model_results[analyte_code] = {
                    "name": analyte_name,
                    "n_bins": actual_bins,
                    "n_normal": n_normal,
            ...
            probe_result["name"] = analyte_name
            probe_result["midpoints"] = [round(m, 3) for m in midpoints]
            model_results[analyte_code] = probe_result

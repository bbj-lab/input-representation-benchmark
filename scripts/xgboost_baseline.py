#!/usr/bin/env python3
"""
XGBoost/LightGBM baseline using standard 24h feature aggregations.

This script constructs a traditional tabular ML baseline from raw MEDS event data:
  1. Extract the first 24h of each hospitalization
  2. Aggregate numeric events per code → min, max, mean, count
  3. Pivot to wide-format feature matrix (one row per admission)
  4. Train XGBoost/LightGBM on four binary outcomes
  5. Evaluate on test set with AUROC/AUPRC and bootstrap 95% CIs
...
from scripts.extract_outcomes_meds import (
...
def _extract_24h_features(meds: pl.LazyFrame) -> pl.DataFrame:
    ...
    # Join admission time to all events
    events = (
        meds.filter(pl.col("hadm_id").is_not_null())
        .collect()
    ...
    # Numeric features: min, max, mean, count per code
    ...
    if numeric_events.height > 0:
        ...
        # First, get the top-N most frequent codes to keep feature matrix manageable
        top_codes = (
            numeric_agg.group_by("code")
            .agg(n=pl.col("hadm_id").count())
            .sort("n", descending=True)
            .head(200)  # Top 200 numeric codes
            .select("code")
        )
        numeric_agg = numeric_agg.join(top_codes, on="code", how="inner")
        
        features_list = []
        for stat_col, stat_name in [("val_min", "min"), ("val_max", "max"),
                                     ("val_mean", "mean"), ("val_count", "count")]:
            pivoted = (
                numeric_agg
                .select("hadm_id", "code", stat_col)
                .pivot(on="code", index="hadm_id", values=stat_col)
                .rename({c: f"{c}__{stat_name}" for c in numeric_agg.select("code").unique().to_series().to_list()
                         if c != "hadm_id"})
            )
    ...
    if cat_events.height > 0:
        ...
        cat_pivot = (
            cat_agg.pivot(on="code", index="hadm_id", values="cat_count")
        )
        # Rename columns
        rename_map = {}
    ...
    # Fill nulls with 0 (missing = event not observed)
...
def main():
    parser = argparse.ArgumentParser(description="XGBoost/LightGBM baseline for EHR prediction.")
    parser.add_argument("--meds_events_dir", type=Path, required=True,
                        help="MEDS events directory with train/val/test splits.")
    parser.add_argument("--output_dir", type=Path, default=Path("outputs/xgboost_baseline"),
                        help="Output directory for results.")
    parser.add_argument("--model", choices=["xgboost", "lightgbm", "auto"], default="auto",
    ...
    for split_name in ("train", "val", "test"):
        ...
        # Check for CLIF raw format
        if (split_dir / "clif_hospitalization.parquet").exists():
            print("    Detected CLIF raw format (adapting to MEDS schema)")
            # 1. Features: Adapt Labs/Vitals/AdmTime to MEDS events
            # All three LazyFrames must have identical schema: hadm_id(Int64), code(String), numeric_value(Float32), time(Datetime)
            ...
            # Inject Hospital Admission event for 24h filtering pivot
            # All columns are created directly in .select() so Polars knows the schema upfront
            ...
            # 2. Outcomes: Compute directly from hospitalization table
            outcomes = pl.scan_parquet(split_dir / "clif_hospitalization.parquet").select(
                pl.col("hospitalization_id").cast(pl.Int64).alias("hadm_id"),
                same_admission_death=pl.col("discharge_category").str.contains("(?i)dead|expire|death").fill_null(False),
                long_length_of_stay=((pl.col("discharge_dttm") - pl.col("admission_dttm")).dt.total_hours() > 7*24).fill_null(False),
                icu_admission=pl.lit(False), # Missing in CLIF raw
                imv_event=pl.lit(False)      # Missing in CLIF raw
            ).collect()
            
        else:
            # Standard MEDS loading
            ...
            outcomes = outcomes.with_columns(pl.col("hospitalization_id").cast(pl.Int64).alias("hadm_id"))
        
        # Join features + outcomes
        merged = features.join(outcomes.select(["hadm_id"] + [c for c, _ in BINARY_OUTCOMES]),
                               on="hadm_id", how="inner")
        
        print(f"    {split_name}: {merged.height:,} admissions, {len(merged.columns) - 1 - len(BINARY_OUTCOMES)} features")
    ...
    for split_name in splits:
        common_features &= set(c for c in splits[split_name].columns if c not in ["hadm_id"] + outcome_cols)
    ...
    for col, display_name in BINARY_OUTCOMES:
        ...
        if len(np.unique(y_train)) < 2:
            print(f"    SKIP: single class in training set")
            continue
        ...
        auroc, auroc_lo, auroc_hi = _bootstrap_ci(y_test, y_score, roc_auc_score, args.n_bootstrap)
        auprc, auprc_lo, auprc_hi = _bootstrap_ci(y_test, y_score, average_precision_score, args.n_bootstrap)
    ...
    print("\n" + "="*60)
    print("SUMMARY TABLE (for paper.tex Table 8)")
    print("="*60)
